...
class ChatUI {
    constructor() {
        console.log('🚀 ChatUI constructor called');
        // Grab references
    ...
    initialize() {
        ...
        if (this.sendButton) {
            ...
            console.log('✅ sendButton listener attached');
        } else {
        ...
        // Inspector button with debug logging
        if (this.inspectorButton) {
            this.inspectorButton.addEventListener('click', async () => {
                ...
                try {
                    ...
                    try {
                        ...
                        console.warn('inject error', error.message);
                    ...
                    if (!port) {
                        console.error('Failed to connect port');
                    } else {
            ...
            console.log('✅ inspectorButton listener attached');
        } else {
            console.warn('inspectorButton element missing when initializing listeners');
        }
    ...
    updateInspectorButtonState() {
        console.log('🔁 updateInspectorButtonState', { isInspecting: this.isInspecting, hasContent: !!this.selectedDomContent });
    ...
    getPromptKeys(language, engine) {
        ...
        // Determine codeGeneratorType for loader text (and store for later)
        const setCodeGenType = (type) => {
            this.codeGeneratorType = type;
            try {
                chrome.storage.sync.set({ codeGeneratorType: type });
            } catch (e) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                setCodeGenType('Cucumber-With-Selenium-Java-Steps');
            } else if (this.isTypeScriptPlaywright(lang, eng)) {
                promptKeys.push('CUCUMBER_WITH_PLAYWRIGHT_TS_STEPS');
                setCodeGenType('Cucumber-With-Playwright-TS-Steps');
            } else {
                // For other combinations, only feature file is supported
                promptKeys.push('CUCUMBER_ONLY');
                this.addUnsupportedLanguageMessage(lang, eng);
                setCodeGenType('Cucumber-Only');
            }
            ...
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('SELENIUM_JAVA_PAGE_ONLY');
                setCodeGenType('Selenium-Java-Page-Only');
            } else if (this.isTypeScriptPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_TS_PAGE_ONLY');
                setCodeGenType('Playwright-TS-Page-Only');
            } else {
                this.addUnsupportedLanguageMessage(lang, eng);
                setCodeGenType('Cucumber-Only');
            }
    ...
    /**
     * Helper to check for Playwright + TypeScript combination
     */
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Only Java/Selenium and Playwright/TypeScript are currently available.`;
    ...
    async resetChat() {
        try {
            ...
            this.isInspecting       = false;
            this.markdownReady      = false;
            console.log('🔄 resetChat resetting inspector button');
            if (this.inspectorButton) {
                this.inspectorButton.classList.remove('has-content','active');
                this.inspectorButton.innerHTML = `
                    <i class="fas fa-mouse-pointer"></i>
                ...
                console.warn('resetChat: inspectorButton not found');
            }
            this.inputField.value = '';
            this.sendButton.disabled = false;
