...
/**
 * Update header elements with app name and logo from config
 */
export function applyAppHeader() {
  const logoEl = document.getElementById('appLogo');
  if (logoEl) {
    logoEl.src = appConfig.logoPath;
  }
  const titleEl = document.getElementById('appTitle');
  if (titleEl) {
    // prefer config appName if available, otherwise fall back to document title
    titleEl.textContent = appConfig.appName || document.title || '';
  }
}
